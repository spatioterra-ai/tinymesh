...
| Does directed MBTA topology add next-headway signal? | On validation, the true upstream arm reaches 145.793 s mean MAE versus 150.272 self-only, 150.682 reversed, and 149.896 degree-preserving permuted controls, winning all three seeds plus route-macro and p90 error. | Freeze validation and open all learned test arms once. Make no topology claim until the test gate passes. |
