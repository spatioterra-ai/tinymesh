...
class PapersTest(unittest.TestCase):
    def test_registry_pins_sources_and_citations(self) -> None:
        ...
        self.assertEqual(set(papers), {"i-jepa", "graph-jepa", "gine", "v-jepa", "ts-jepa"})
    ...
    def test_cli_lists_and_cites_every_paper(self) -> None:
        ...
        self.assertIn("ts-jepa      2509.25449v1", listing)
        self.assertEqual(len(re.findall(r"^@", citations, re.MULTILINE)), 5)
        ...
        self.assertIn("@inproceedings{Ennadir_2024_TSJEPA,", citations)
